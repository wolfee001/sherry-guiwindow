...
{
	// Functions
	bool ImGuiGLConnector::ImGui_ImplOpenGL3_Init(const char* glsl_version)
	{
...
#endif
		IM_ASSERT((int)strlen(glsl_version) + 2 < IM_ARRAYSIZE(mGlslVersionString));
		strcpy(mGlslVersionString, glsl_version);
		strcat(mGlslVersionString, "\n");
	...
	void ImGuiGLConnector::ImGui_ImplOpenGL3_Shutdown()
	{
		ImGui_ImplOpenGL3_DestroyDeviceObjects();
	}

	void ImGuiGLConnector::ImGui_ImplOpenGL3_NewFrame()
	{
		if (!mFontTexture)
	...
	void ImGuiGLConnector::ImGui_ImplOpenGL3_RenderDrawData(ImDrawData * draw_data)
	{
	...
	bool ImGuiGLConnector::ImGui_ImplOpenGL3_CreateFontsTexture()
	{
		// Build texture atlas
		ImGuiIO& io = ImGui::GetIO();
	...
	void ImGuiGLConnector::ImGui_ImplOpenGL3_DestroyFontsTexture()
	{
		if (mFontTexture)
		{
			ImGuiIO& io = ImGui::GetIO();
	...
	bool ImGuiGLConnector::CheckShader(GLuint handle, const char* desc)
	{
	...
	bool ImGuiGLConnector::CheckProgram(GLuint handle, const char* desc)
	{
		GLint status = 0, log_length = 0;
		glGetProgramiv(handle, GL_LINK_STATUS, &status);
		glGetProgramiv(handle, GL_INFO_LOG_LENGTH, &log_length);
		if ((GLboolean)status == GL_FALSE)
			fprintf(stderr, "ERROR: ImGui_ImplOpenGL3_CreateDeviceObjects: failed to link %s! (with GLSL '%s')\n", desc, mGlslVersionString);
		if (log_length > 0)
		{
			ImVector<char> buf;
			buf.resize((int)(log_length + 1));
	...
	bool ImGuiGLConnector::ImGui_ImplOpenGL3_CreateDeviceObjects()
	{
		// Backup GL state
	...
	void ImGuiGLConnector::ImGui_ImplOpenGL3_DestroyDeviceObjects()
	{
